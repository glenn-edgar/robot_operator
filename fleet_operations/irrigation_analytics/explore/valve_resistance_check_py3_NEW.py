...
class Valve_Resistance_Check(object):
   ...
   def valve_measurement(self, cf_handle, chainObj, parameters, event ):
       ...
       # Three back-to-back ACS712 samples averaged, per Glenn 2026-06-09 PM.
       # Keeps the original code's continuous-bus timing (required for
       # parallel-pair valves like sat_1:44 to latch) but cuts measurement
       # noise by ~sqrt(3) vs the prior single-sample read.
       s1 = self.io_control.measure_valve_current()
       s2 = self.io_control.measure_valve_current()
       s3 = self.io_control.measure_valve_current()
       coil_current = (s1 + s2 + s3) / 3.0

       print("coil_current",self.irrigation_current_limit,coil_current,"samples=",s1,s2,s3)
