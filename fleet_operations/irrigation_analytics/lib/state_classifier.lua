-- lib/state_classifier.lua — popup → state string + edge detection.
--
-- Port of irrigation_analytics/robot/lib/state_machine.lua. Single source of
-- truth for both monitor and detector chains. Pure functions, no I/O.
...
-- Used by the detector chain to enable KB2 chains and similar one-shots.
function M.edges(prev_state, cur_state)
    ...
    -- run KB2 even when the next state is CLEAN_FILTER.
