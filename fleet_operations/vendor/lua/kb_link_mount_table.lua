--[[
  kb_link_mount_table.lua — LuaJIT port of KB_Link_Mount_Table

  Query operations on the knowledge_base_link_mount table.
  Provides lookups by link_name, mount_path, and enumeration
  of all unique link names and mount paths.
  ...
  Usage:
    ...
    local mounts = KB_Link_Mount_Table.new(db, 'knowledge_base')

    local recs = mounts:find_records_by_link_name('my_link')
    local all  = mounts:find_all_mount_paths()
]]
...
--- Create a new KB_Link_Mount_Table instance.
-- @param db          sqlite3* FFI handle
-- @param database    base name (suffixed with '_link_mount')
function KB_Link_Mount_Table.new(db, database)
...
function KB_Link_Mount_Table:find_records_by_link_name(link_name, kb)
    if kb then
        return sql_query_rows(self.db,
            string.format("SELECT * FROM %s WHERE link_name = ? AND knowledge_base = ?", self.base_table),
            { link_name, kb })
    else
        return sql_query_rows(self.db,
...
function KB_Link_Mount_Table:find_records_by_mount_path(mount_path, kb)
    if kb then
        return sql_query_rows(self.db,
            string.format("SELECT * FROM %s WHERE mount_path = ? AND knowledge_base = ?", self.base_table),
            { mount_path, kb })
    else
        return sql_query_rows(self.db,
            string.format("SELECT * FROM %s WHERE mount_path = ?", self.base_table),
            { mount_path })
    end
end
...
function KB_Link_Mount_Table:find_all_mount_paths()
    local rows = sql_query_rows(self.db,
        string.format("SELECT DISTINCT mount_path FROM %s ORDER BY mount_path", self.base_table), {})
    local rv = {}
    for _, r in ipairs(rows) do if r.mount_path then rv[#rv + 1] = r.mount_path end end
    return rv
end
